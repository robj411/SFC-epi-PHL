  ...
  for (i in 2:nPeriods-1){
    ...
    for (iterations in 1:100){
      ...
      #Behavioural equations and equilibrium condition
        ...
        l_s[j,i+1] = l_s[j,i] + l_d[j,i+1] - l_d[j,i]  
      
      #Transactions of the firms
      
        #GDP - eq. 7.5
        y[j,i+1] = c_s[j,i+1] + i_s[j,i+1]
        #Wage bill - eq. 7.6
        wb_d[j,i+1] = y[j,i+1] - rl[j,i]*l_d[j,i] - af[j,i+1]   
        #Depreciation allowances - eq. 7.7
        af[j,i+1] = da[j,i+1]
        #Demand for bank loans - eq. 7.8
        l_d[j,i+1] = l_d[j,i] + i_d[j,i+1] - af[j,i+1] 
      
      #Transactions of households
      
        #Disposable income - eq. 7.9
        yd[j,i+1] = wb_d[j,i+1] + rm[j,i]*mh[j,i]
        
        #Bank deposits held by households - eq. 7.10
        mh[j,i+1] = mh[j,i] + yd[j,i+1] - c_d[j,i+1]
        
      #Transactions of the banks  
      
        #Supply of deposits - eq. 7.11
        ms[j,i+1] = ms[j,i] + l_s[j,i+1] - l_s[j,i]
      ...
      #The wage bill  
        ...
        wb_s[j,i+1] = w[j,i+1]*n_s[j,i+1]         
      
        #Labour demand - eq. 7.14
        n_d[j,i+1] = y[j,i+1]/pr[j,i+1]        
      
        #Wage rate - eq. 7.15
        w[j,i+1] = wb_d[j,i+1]/n_d[j,i+1]       
      
      #Household behaviour
      
        #Demand for consumption goods - eq. 7.16 (and 7.16A)
        if (j==5){ 
          c_d[j,i+1] = alpha0[j,i+1] + alpha1w*wb_d[j,i+1] + alpha1r*rm[j,i]*mh[j,i] + alpha2*mh[j,i]
        }else{ 
          c_d[j,i+1] = alpha0[j,i+1] + alpha1[j,i+1]*yd[j,i+1] + alpha2*mh[j,i]
        }
      
      #The investment behaviour
      
        #Accumulation of capital - eq. 7.17
        k[j,i+1] = k[j,i] + i_d[j,i+1] - da[j,i+1]
      
        #Depreciation allowances - eq. 7.18
        da[j,i+1] = delta*k[j,i]
      
        #Capital stock target - eq. 7.19
        kt[j,i+1] = kappa[j,i+1]*y[j,i]
      
        #Demand for investment goods - eq. 7.20
        i_d[j,i+1] = gamma*(kt[j,i+1] - k[j,i]) + da[j,i+1]
      
      #The behaviour of banks
      
        #Interest rate on loans - eq. 7.21
        rl[j,i+1] = rl_bar[j,i+1]
      
      #Additional equation for calculating y steady-state value
        y_star[j,i+1] = alpha0[j,i+1]/((1-alpha1[j,i+1])*(1-delta*kappa[j,i+1])-alpha2*kappa[j,i+1])       
...
aerror=0
error=0
for (i in 2:(nPeriods-1)){error = error + (ms[1,i+1]-mh[1,i+1])^2}
