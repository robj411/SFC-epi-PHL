...
for (j in 1:nScenarios){
  ...
  for (i in 2:nPeriods-1){
    ...
    for (iterations in 1:100){
      ...
      if (i>=10 && j==2){r_bar[j,i+1]=0.035}
      
      #Shock 2: higher propensity to consume out of income
      if (i>=10 && j==3){alpha1[j,i+1]=0.7}
      
      
      #STEP 4.C: DEFINE THE SYSTEM OF EQUATIONS        
      
      #Determination of output - eq. 4.1
      y[j,i+1] = cons[j,i+1] + g[j,i+1]
      
      #Disposable income - eq. 4.2
      yd[j,i+1] = y[j,i+1] - t[j,i+1] + r[j,i]*b_h[j,i]
      
      #Tax payments - eq. 4.3
      t[j,i+1] = theta*(y[j,i+1] + r[j,i]*b_h[j,i])
      
      #Wealth accumulation - eq. 4.4
      v[j,i+1] = v[j,i] + (yd[j,i+1] - cons[j,i+1])
      
      #Consumption function - eq. 4.5
      cons[j,i+1] = alpha1[j,i+1]*yd[j,i+1] + alpha2*v[j,i]
      
      #Cash money - eq. 4.6
      h_h[j,i+1] = v[j,i+1] - b_h[j,i+1]
      
      #Demand for government bills - eq. 4.7
      b_h[j,i+1] = v[j,i+1]*(lambda0 + lambda1*r[j,i+1] - lambda2*(yd[j,i+1]/v[j,i+1]))
      
      #Supply of government bills - eq. 4.8
      b_s[j,i+1] = b_s[j,i] + (g[j,i+1] + r[j,i]*b_s[j,i]) - (t[j,i+1] + r[j,i]*b_cb[j,i])
      
      #Supply of cash - eq. 4.9
      h_s[j,i+1] = h_s[j,i] + b_cb[j,i+1] - b_cb[j,i]
      
      #Government bills held by the central bank - eq. 4.10
      b_cb[j,i+1] = b_s[j,i+1] - b_h[j,i+1]
...
aerror=0
error=0
for (i in 2:(nPeriods-1)){error = error + (h_s[1,i+1]-h_h[1,i+1])^2}
